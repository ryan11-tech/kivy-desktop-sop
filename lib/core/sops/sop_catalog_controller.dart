...
import 'sop_category_mapper.dart';
import 'sop_repository.dart';

enum SopCatalogStatus {
  loading,
  loaded,
  empty,
  permissionDenied,
...
/// Loads and exposes the SOP catalog for the currently selected shop.
///
/// Reloads when the active shop id changes and clears the previous shop's data
/// before loading, so a slow response can never leak into another shop's view.
class SopCatalogController extends ChangeNotifier {
  SopCatalogController(this._repository);
  ...
  /// Loads SOPs for [shop]. No-op when the shop is unchanged and the last load
  /// succeeded, unless [force] is set.
  ...
  Future<void> _load(String shopId) async {
    ...
    try {
      final items = await _repository.listShopSops(shopId);
      if (_isStale(token)) return;
      _groups = groupBySopType(items);
      _setStatus(
